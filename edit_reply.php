...
$cid = $_GET['cid'];	//catogory id
$tid = $_GET['tid'];	//thread id
$pid = $_GET['pid'];		//post id to get the old post contents


$stmt = $mysqli->prepare("SELECT post_content FROM posts WHERE id = ?");//select from the post in the db which id matches the pid
//that was passed through via the php script on the previous page
$stmt->bind_param('i', $pid);
$stmt->execute();
$stmt->bind_result($post_content);
$stmt->store_result();	//grab the old post content for user to edit
$stmt->fetch();

?>
<div id="content">
 <form action="edit_reply_parse.php" method="post">
 <p>Edit Reply</p>
 <textarea name="edit_content" rows="5" cols="75"><?php echo "{$post_content}"?></textarea>	
 <br  /> <br />
 <input type="hidden" name="cid" value="<?php echo $cid;?>"/>
 <input type="hidden" name="pid" value="<?php echo $pid;?>"/>
 <input type="hidden" name="tid" value="<?php echo $tid;?>"/>
 <input type="submit" name="reply_submit" value="Save changes" />
</div>
</div>
</body>
</html>
